#!/home/vmuser/myenv/bin/python
import math
import os
import shutil
import base64 #Aggiuto da Baso
import shutil #Aggiuto da Baso
import essentia
...
#Baso <
def request(artist,title,audiob64):
    #diffs = ['Beginner', 'Easy', 'Medium', 'Hard', 'Challenge']
    diffs = ['Beginner']
    result=create_chart_closure(artist, title, audiob64, diffs)
    return result
#Baso >
...
#Baso Matteo <
            ...
            #print 'AudioFP:{}'.format(audio_fp)
            print 'Diffs: {}'.format(diffs)
            
            # Decodifica il contenuto audio base64
            try:
                audio_fp_str = str(audio_fp)
                print 'AudioFP:{}'.format(audio_fp_str[:100])
                decoded_audio_content = base64.b64decode(audio_fp_str)
                print 'AudioFPDecoded:{}'.format(decoded_audio_content[:20])
            except Exception, e:
                print("Errore durante la decodifica base64:", str(e))
                print(type(audio_fp))

            print 'test 1'
            # Salva il file audio nella cartella desiderata
            audio_filename = str(song_id) + '.mp3'  # Nome del file audio
            audio_fp = os.path.join('/home/vmuser/ddc_infer/temp_audio_request/', audio_filename)  # Percorso completo del file audio
            print 'test 2 audio file path = {}'.format(audio_fp)
            with open(audio_fp, 'wb') as audio_file:
...
#Baso Matteo <    
            ...
            #processo di salvataggio file .sm base64 nella variabile da inserire nel return e rimozione file salvati in /ddc_infer e /temp_audio_request
            try:
                print 'test 5'
                smfilepath = os.path.join(args.out_dir, str(song_id),str(song_id)+'.sm')
                print 'smfilepath :{}'.format(smfilepath)
                # Leggi il contenuto del file
                with open(smfilepath, 'rb') as file:
                ...
                cartella_da_eliminare = os.path.join('/home/vmuser/ddc_infer/', str(song_id)) 
                try:
                    # Elimina la cartella e tutto il suo contenuto
                    shutil.rmtree(cartella_da_eliminare)
                    os.remove(audio_fp)
                    print 'Files eliminati con successo.'
                except Exception as e:
                    print ("Si e' verificato un errore durante la cancellazione:", str(e))


                # Restituisci il contenuto codificato in base64 come parte della risposta XML-RPC
                return {'file_content': base64_content}
            except Exception as e:
                # Gestisci eventuali errori
                return {'error': str(e)}
            
#Baso Matteo >
            #return os.path.abspath(out_zip_fp) Modificato da Baso

        print 'Waiting for RPCs on port {}'.format(args.port)
        #server = SimpleXMLRPCServer(('localhost', args.port)) Modificato da Baso
        server = SimpleXMLRPCServer(('192.168.56.101', args.port))
        #server.register_function(create_chart_closure, 'create_chart')Modificato da Baso
